...
===============
*/
static void Upload32(int numTexLevels, const textureLevel_t *pics, int x, int y, int width, int height, GLenum picFormat, GLenum dataFormat, GLenum dataType, image_t *image, qboolean scaled, int picmip)
{
	int			i, c;
	byte		*scan;
	int			baseLevel;
...
image_t *R_CreateImage2( const char *name, int numTexLevels, const textureLevel_t *pics,
	...
	if (resampledBuffer != NULL) {
		...
		Upload32(1, &texLevel, 0, 0, width, height, picFormat, dataFormat, dataType, image, scaled, picmip);

		ri.Hunk_FreeTempMemory(resampledBuffer);
	}
	else if (pics[0].data) {
		Upload32(numTexLevels, pics, 0, 0, width, height, picFormat, dataFormat, dataType, image, scaled, picmip);
	}
...
void R_UpdateSubImage( image_t *image, byte *pic, int x, int y, int width, int height, GLenum picFormat )
{
	...
	Upload32(1, &texLevel, x, y, width, height, picFormat, dataFormat, dataType, image, qfalse, 0);
}
